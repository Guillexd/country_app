...
export default function Header({ countries }) {
    ...
    const handleSearch = async (e) => {
        e.preventDefault()
        const value = await getCountryByName(search)
        value.hasOwnProperty('alpha3Code') && navigate(`/country/${value.alpha3Code}`)
    }
            ...
            <div className='w-full sm:w-1/3'>
                <label className='relative'>
                    ...
                    <form onSubmit={handleSearch}>
                        <input list='countries' className='placeholder:italic placeholder:text-slate-400 block bg-white dark:bg-[#2b3945] w-full shadow-lg rounded-md py-2 pl-14 pr-3 focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm h-14 text-dark dark:text-white' placeholder='Search for a country ...' 
                        type='text'
                        value={search}
